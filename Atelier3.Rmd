...
```{r}
nutriage <- nutriage <- read.csv("~/Downloads/nutriage.csv")
length(nutriage)
names(nutriage)
```
```{r}
...
Consomation de viande chez les et chez les femmes
```{r}
...
```
```{r}
pie(table(viande.f),main="c.viande chez les femmes")
```
```{r}
pie(table(nutriage$viande),main="c.viande chez les populations")
```
```{r}
str(nutriage)
nutriage$sexe
nutriage$chocol
```
Consomation de chocolat chez les et chez les femmes
```{r}
...
```
```{r}
pie(table(chocol.f),main="c.chocol chez les femmes")
```
```{r}
pie(table(nutriage$chocol),main="c.chocolat chez les populations")
```
Consomation de chocolat chez les et chez les femmes
```{r}
chocol.h <- nutriage$chocol[nutriage$sexe==1]
